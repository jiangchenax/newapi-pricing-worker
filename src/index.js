...
function modelVariants(value) {
  ...
  // Custom gateway names often prepend a site/channel prefix.
  // Generate suffixes so:
  // blian_deepseek_v4_pro -> deepseek-v4-pro
  for (
    let i = 1;
    i < Math.min(
      tokens.length - 1,
  ...
  // Hosting suffix, not necessarily part of the canonical model.
  if (
...
async function getModelsDevRecords() {
  ...
  if (
    modelsDevMemoryCache.records &&
    modelsDevMemoryCache.expiresAt > now
  ) {
    return modelsDevMemoryCache.records;
  }
  ...
  const records =
    collectModelRecords(payload);

  modelsDevMemoryCache = {
    records,
    expiresAt:
      now + 6 * 60 * 60 * 1000,
  };
...
function scoreCandidate(
  ...
  for (
    let sourceIndex = 0;
    sourceIndex < searchValues.length;
    sourceIndex++
  ) {
    const source =
      searchValues[sourceIndex];
...
function findBestModelRecord(
  ...
  // Below this threshold, do not guess.
  if (
    !best ||
    best.score < 90
  ) {
...
async function loadAdminMappingIndex(
  ...
  if (
    ...
    adminMappingMemoryCache.expiresAt > now
  ) {
...
function sanitizeMetadata(
  ...
  return {
    ...
    last_updated:
      raw.last_updated || "",
    open_weights:
      typeof raw.open_weights === "boolean"
        ? raw.open_weights
        : null,
    reasoning:
    ...
    attachment:
      typeof raw.attachment === "boolean"
        ...
        : null,
    temperature:
      typeof raw.temperature === "boolean"
        ? raw.temperature
        : null,
    limit: {
      context:
...
async function handlePricingMeta(
  ...
  sources.push({
    ...
    kind:
      "本站模型目录",
    note:
      "模型名称、分组、标签、端点和运行状态仍以本站 New API 为准。",
  });
  ...
  if (
    ...
    sources.push({
      ...
      kind:
        "本站映射",
      note:
        "由服务器端管理员接口解析调用名对应的实际上游模型；管理员凭据未下发到浏览器。",
    });
  }
  ...
  if (metadata) {
    ...
    sources.push({
      ...
      url:
        modelsDevURL,
      note:
        "用于上下文、输出限制、模态和能力等模型规格；这是开源社区维护的数据源。",
    });

    if (
      metadata.official_link
    ) {
      sources.push({
        name:
          "官方模型资料",
        ...
        url:
          metadata.official_link,
        note:
          "Models.dev 元数据中提供的模型/实验室官方资料链接。",
      });
    }
  }
...
class HeadInjector {
  element(element) {
    element.append(
      '<style id="moss-pricing-v5-style">' +
      GLOBAL_CSS +
      '</style>',
      {
        html: true,
...
class BodyInjector {
  element(element) {
    ...
    element.append(
      '<script id="moss-pricing-v5-script">' +
      safeClientJs +
      '</script>',
      {
        html: true,
...
export default {
  async fetch(
    ...
    return withDebugHeader(
      transformed,
      "active-v5-source-backed-details"
    );
  },
};
