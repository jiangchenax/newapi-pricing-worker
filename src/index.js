...
// unified detail tabs + single scroll + shared perf + icon resolver
// ============================================================
...
const PRICING_ICON_ALIASES = {
  ...
  claude: {
    lobe: ["claude", "anthropic"],
    simple: ["anthropic"],
  },

  anthropic: {
    lobe: ["anthropic", "claude"],
    simple: ["anthropic"],
  },

  gemini: {
    lobe: ["gemini"],
    simple: ["googlegemini", "google"],
  },
  ...
  chatglm: {
    lobe: ["chatglm", "qingyan", "zhipu"],
    simple: [],
  },

  zhipu: {
    lobe: ["zhipu", "qingyan", "chatglm"],
    simple: [],
  },
...
async function fetchPricingIcon(
  ...
  /*
   * Lobe static SVG supports normal and many "-color" variants.
   * Try color first, then standard.
   */
  const lobeCandidates =
    uniqueStrings(
      aliases.lobe.flatMap(
        (slug) => [
          slug + "-color",
          slug,
        ]
    ...
    const response =
      await fetch(
          ...
          ".svg",
        {
          headers: {
            Accept:
              "image/svg+xml,image/*;q=0.9,*/*;q=0.8",
          },
  ...
  /*
   * Secondary brand source: Simple Icons color CDN.
   */
...
class HeadInjector {
  element(element) {
    element.append(
      '<style id="moss-pricing-v71-style">' +
      GLOBAL_CSS +
      '</style>',
      {
        html: true,
...
class BodyInjector {
  element(element) {
    ...
    element.append(
      '<script id="moss-pricing-v71-script">' +
      safeClientJs +
      '</script>',
      {
        html: true,
...
export default {
  async fetch(
    ...
    return withDebugHeader(
      transformed,
      "active-v7.1-unified-tabs-scroll-perf-icons"
    );
  },
};
